...
The expected difference in Pr is 0.46/4=0.115, se is 0.04/4=0.01.
For difference = 0.5, 0.5*0.115=0.0575, 0.005 se, 
0.0575+-2*0.005 = [0.475, 0.0675]
...
reg_7 <- stan_glm(formula = heavy ~ height, family = binomial(link="logit"), 
                  data = health, refresh = 0)  
plot(health$height, heavy)
curve(invlogit(coef(reg_7)[1] + coef(reg_7)[2]*x), add = TRUE)
```
...
reg_8 <- stan_glm(formula = heavy ~ height, family = binomial(link="logit"), 
                  data = health, refresh = 0)
reg_8
```
...
fit_10 <- stan_glm(y~x, family = binomial(link="logit"), refresh=0)
print(fit_10)
```
...
The estimate is 0.5-0.4=0.1, the se is sqrt(0.5^2/500+0.5^2/500)=0.03
The regression coefficient is approximately 0.4, 
using the divide-by-4 rule, standard error is 0.4/4 = 0.1.
...
hispanic <- rodents$hispanic_Mean * 10
black <- rodents$black_Mean * 10

reg_11 <- stan_glm(rodent2 ~ race + hispanic + black, 
                   data = rodents, family = binomial(link="logit"), refresh = 0)
print(reg_11)
...
```{r}
reg_11.b <- glm(rodent2 ~ race + hispanic + black + borough + old + housing + 
                  personrm + struct + foreign, data=rodents, 
...
```{r}
reg_14.3 <- stan_glm(switch ~ dist, data = wells, 
...
```{r}
library(arm)
plot(predict(reg_14.3),residuals(reg_14.3), main="Residual plot", 
     xlab="Expected Values", ylab="Residuals")
binnedplot(predict(reg_14.3),residuals(reg_14.3))
```
...
```{r}
predicted <- predict(reg_14.3)
s <- wells$switch
mean((predicted>0.5 & s==0) | (predicted<0.5 & s==1))

predicted.null <- seq(0, 0, length.out=length(y))
mean((predicted.null>0.5 & s==0) | (predicted.null<0.5 & s==1))
```
...
reg_e <- stan_glm(switch ~ dist_lt100 + dist_gte100_lt200 + dist_gte200, 
...
reg_14.7 <- stan_glm(switch ~ dist * log.arsenic, family=binomial(link="logit"), 
...
```{r}
ggplot(data=wells, aes(x=dist, y=switch)) + geom_jitter(position=position_jitter(height=.05)) + 
  geom_smooth(method="glm", family="binomial")

ggplot(data=wells, aes(x=log.arsenic, y=switch)) + geom_jitter(position=position_jitter(height=.05)) + 
  geom_smooth(method="glm", family="binomial")

ggplot(data=wells, aes(x=dist, y=log.arsenic)) + geom_jitter(position=position_jitter(height=.05)) + 
  geom_smooth(method="glm", family="binomial")
```
...
hi <- 100
lo <- 0
delta <- invlogit(b[1] + b[2]*hi + b[3]*wells$log.arsenic + 
                    b[4]*wells$log.arsenic*hi) - invlogit(b[1] + b[2]*lo + b[3]*wells$log.arsenic + b[4]*wells$log.arsenic*lo)
print(mean(delta))
...
hi <- 1.0
lo <- 0.5
delta <- invlogit(b[1] + b[2]*wells$dist + b[3]*hi + b[4]*wells$dist*hi) - 
  invlogit(b[1] + b[2]*wells$dist + b[3]*lo + b[4]*wells$dist*lo)
print(mean(delta))
